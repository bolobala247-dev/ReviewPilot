...
interface IAIProvider {
  ...
  review(request: { systemPrompt: string; userPrompt: string; temperature?: number }): Promise<{
    content: string;
    tokensUsed: number;
    model: string;
  }>;
...
function createProvider(config: AppConfig): IAIProvider {
  switch (config.ai.provider) {
    ...
    default:
      throw new AppError(ErrorCode.CONFIG_ERROR, `Unknown provider: ${config.ai.provider}`);
  }
}
```
...
function createProvider(config: AIConfig): IAIProvider {
  switch (config.provider) {
    ...
    default:
      throw new AppError(ErrorCode.CONFIG_ERROR, `Unknown provider: ${config.provider}`);
  }
}
```
...
const ConfigSchema = z.object({
  ...
  review: z.object({
    ...
    ignorePatterns: z
      .string() // Comma-separated, parsed into string[]
...
export function loadConfig(): AppConfig {
  return ConfigSchema.parse({
    ai: {
      ...
      temperature: process.env.AICR_AI_TEMPERATURE
        ? Number(process.env.AICR_AI_TEMPERATURE)
        : undefined,
    },
    github: {
